...
- `https://getknctd.com` responds over HTTPS and redirects to `https://www.getknctd.com/`, which serves the Biz Card application. Project ownership and OAuth configuration still need dashboard verification.
- Google connection and one scheduled send were previously confirmed. Repeat on the final TestFlight build.
- Microsoft has an existing KNCT registration supporting all Microsoft account users. The dashboard currently lists only delegated User.Read, no client credentials, and the old Vercel callback. Registration is not equivalent to a working Outlook connection.
...
| 2 | Domain | HTTPS verified; dashboard pending | Domain belongs to bizcard project, correct production deployment, email DNS preserved |
| 3 | URLs and redirects | Pending | Coordinated provider, Supabase, server, scheduler and mobile configuration; return to installed app |
| 4 | Gmail | Partial live proof | Tester access, connect/cancel/reconnect, token refresh and send from final build |
| 5 | Outlook | Configuration incomplete | Delegated permissions, server credentials, callbacks; personal Outlook and Microsoft 365 live tests |
...
Microsoft registration currently uses the old callback `/api/mailbox/callback/microsoft` on bizcard-nu.vercel.app. Add delegated Mail.Send, openid, email and offline_access while retaining User.Read. No inbox-reading or application-wide mailbox permission is needed. Store client credentials only on the web server. Do not grant tenant-wide consent as a substitute for testing individual user consent.
